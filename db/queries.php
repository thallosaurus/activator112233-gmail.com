...
= "SELECT coordinates.id, coordinates.db_lat AS db_lat, coordinates.db_lon AS db_lon, posts.title, posts.category, posts.type, ROUND(6371 * ACOS( COS( RADIANS( db_lat ) ) * COS( RADIANS( :u_lat ) ) * COS( RADIANS( :u_lon ) - RADIANS( db_lon ) ) + SIN( RADIANS( db_lat ) ) * SIN( RADIANS( :u_lat ) ) ), 4) AS distance FROM coordinates JOIN posts ON coordinates.id = posts.id HAVING distance <= :u_rad ORDER BY distance ASC";
...
$GLOBALS["lm"]["QRY"]["create_cat"]
    = "INSERT INTO `categories`(`value`) VALUES (:name)";
?>